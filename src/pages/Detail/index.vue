    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          />
          <SpecScroll :skuImageList="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
export default {
  ...
  methods: {
    // 选择商品参数
    changeChecked(saleAttr, supSaleAttrList) {
      supSaleAttrList.forEach((item) => {
        item.isChecked = "0";
      });
    ...
    async addShopCart() {
      await this.$store
        ...
        .then(
          ...
          () => {
            // 遍历出选择的商品参数
            this.spuSaleAttrList.forEach((items) => {
              items.spuSaleAttrValueList.forEach((item) => {
                if (item.isChecked == 1) this.attr.push(item);
              });
            });
            // 将选择的商品数据存储到浏览器
