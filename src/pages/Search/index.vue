        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-if="searchParams.categoryName">
              ...
              <i @click="removeCategoryName" class="el-icon-close"></i>
            </li>
            <!-- 关键字分类 -->
            <li class="with-x" v-if="searchParams.keyword">
              {{ searchParams.keyword }}
              <i @click="removeKeyword" class="el-icon-close"></i>
            </li>
            <!-- 品牌分类 -->
            <li class="with-x" v-if="searchParams.trademark">
              {{ searchParams.trademark.split(":")[1] }}
              <i @click="removeTrademark" class="el-icon-close"></i>
            </li>
            <!-- 商品属性分类 -->
            <li
              class="with-x"
            ...
            >
              {{ attr.split(":")[1] }}
              <i @click="removeAttr(index)" class="el-icon-close"></i>
            </li>
          </ul>
        </div>
        <!--SearchSelector-->
        <SearchSelector
          @getDrademarkInfo="getDrademarkInfo"
          @getAttrValueInfo="getAttrValueInfo"
        />
        <!--details-->
        <div class="details clearfix">
          ...
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeHandle('1')">
                  <a>
                    综合
                    <i
                      v-show="isOne"
                ...
                <li :class="{ active: isTwo }" @click="changeHandle('2')">
                  <a>
                    价格
                    <i
                      v-show="isTwo"
          ...
          <h2 v-if="total == 0">暂无该商品！看看其它的吧．</h2>
        </div>
        <Pagination
...
export default {
  ...
  methods: {
    // 获取 searchInfo 数据
    getData(params) {
      // 选择的当前页也 不能小于1、大于页面总数、非数字、非空和非零
      if (!params.pageNo) params.pageNo = 1;
      if (isNaN(params.pageNo * 1)) params.pageNo = 1;
      if (params.pageNo > this.pageTotal) {
        params.pageNo = this.pageTotal;
    ...
    changeHandle(index) {
      ...
      if (originIndex == index) {
        order = `${index}:${this.isUp ? "desc" : "asc"}`;
      } else {
        order = `${index}:"desc"`;
      }
  ...
  watch: {
    ...
    $route() {
      ...
      // 发送请求之前, 整理参数
      this.$route.query.pageNo = parseInt(this.$route.query.pageNo);
  ...
  beforeMount() {
    // 发送请求之前, 整理参数
    let pageNo = parseInt(this.$route.query.pageNo);
    if (!pageNo) {
      pageNo = 1;
    }
    if (isNaN(pageNo * 1)) {
    ...
    this.$route.query.pageNo = pageNo;
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
    // 挂载完成之后发送请求
    this.getData(this.searchParams);
  },
